...
assert(/Step 2: screenshot inspected/.test(app) && /Screenshot inspection required before retry/.test(app) && /Inspect screenshot first/.test(app) && /Object still wrong: retry same object/.test(app) && /First fallback: ask LLM for new hand object/.test(app) && /Replacement object wrong: retry replacement/.test(app) && /Replacement still bad: render without object/.test(app) && /Visual fallback sequence/.test(app) && /heldObjectFallback/.test(app) && /visualRetryPlan/.test(bridge) && /retry-same-object/.test(bridge) && /replacement-held-object/.test(bridge) && /drop-held-object/.test(bridge) && /dropHeldObjectsFromReactCssCharacter/.test(bridge), 'test5 must expose a durable visual quality loop: screenshot inspection first, retry the same unclear held object 3 times, ask LLM for a replacement hand object and retry that 3 times, then drop held objects');
