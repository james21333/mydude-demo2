...
assert(/Step 2: screenshot inspected/.test(app) && /Screenshot inspection required before retry/.test(app) && /Inspect screenshot first/.test(app) && /Object still wrong: retry same object/.test(app) && /Third try still bad: render without object/.test(app) && /heldObjectFallback/.test(app) && /visualRetryPlan/.test(bridge) && /retry-same-object/.test(bridge) && /drop-held-object/.test(bridge) && /dropHeldObjectsFromReactCssCharacter/.test(bridge), 'test5 must expose a durable visual quality loop: screenshot inspection first, then retry the same unclear held object up to 3 times, then drop it and render without the object');
