...
assert(/Prompt-pass cue question/.test(bridge) && /Prompt-pass cue decision/.test(bridge) && /Prompt-pass cue fallback/.test(bridge) && /prompt-pass-cue/.test(bridge) && /head, hand\/hands, feet, clothing\/body, neck/.test(bridge) && /female cue: ask what to put on the head/.test(bridge) && /single stray capsule\/blob on one foot/.test(bridge) && /footwear must be paired left\+right/.test(bridge) && /stray foot\/hand artifacts|stray artifacts around the feet\/hands/.test(bridge), 'bridge must ask a generic screenshot-pass cue question for every prompt, run prompt-pass cue fallback attempts, require female head cues when relevant, and inspect head/hands/feet/clothing/neck zones instead of only held objects');
assert(/Prompt still fails: ask LLM for visible cue/.test(app) && /prompt-pass cue fallback 3 tries on head\/hands\/feet\/clothing\/neck/.test(app), 'frontend must expose the post-object prompt-pass cue fallback loop');
