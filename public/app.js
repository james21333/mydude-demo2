...
function sendToBridge(userText) {
  ...
  socket.onmessage = (ev) => {
    ...
    if (p.type === 'ready') {
      setStatus('building');
      showMsg('Thinking…');
      const instruction = `You are a 3D scene JSON API. RESPOND ONLY with a short spoken reply, then a \`\`\`json code block. Never skip the \`\`\`json block. Schema: [{id,assetPath,action:"add"|"update"|"remove",position:{x,y,z},rotation:{x,y,z},scale:{x,y,z},label}]. Y=0=ground, rotation in degrees. For assetPath use "kenney:WORDS" with 1-3 words from this exact vocabulary: WEAPONS→ blaster; CHARACTERS→ character; VEHICLES→ ambulance firetruck police sedan race suv hatchback kart delivery; CITY→ building skyscraper office store house suburban; FACTORY→ factory warehouse conveyor; MEDIEVAL→ sword shield chest blade cart banner wall; HORROR→ zombie ghost vampire skeleton gravestone coffin pumpkin candle crypt; DUNGEON→ barrel orc chest shield trap gate; CAVE→ cave rock; SPACE→ space corridor room gate stairs scifi; PIRATE→ cannon ship boat treasure; PLATFORMER→ platform coin; ANIMALS→ cat dog bunny. Examples: "kenney:zombie", "kenney:ghost", "kenney:vampire", "kenney:gravestone", "kenney:pumpkin", "kenney:coffin", "kenney:blaster", "kenney:character", "kenney:sedan", "kenney:ambulance", "kenney:firetruck", "kenney:cannon", "kenney:ship", "kenney:space corridor", "kenney:cave rock", "kenney:orc", "kenney:barrel", "kenney:cat", "kenney:dog". If no scene change, output []. ALWAYS end with the \`\`\`json block.`;
